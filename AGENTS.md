...
Repository baseline as of 2026-09-14:

- `web/` contains a responsive React/TypeScript/Vite Dashboard, doctor schedule workspace, and My Patients directory with explicit synthetic fixtures, practice-scoped directory search/filtering/pagination, add/edit patient forms with in-memory demo saves, patient previews, global patient search, queue previews, inbox filters, practice-scoped patient charts, and historical encounter screens with guarded in-memory note editing. The provider-support workspace can create a local Practice tenant, prepare its initial administrator identity, persist its first Philippine clinic location through the Spring API, list/search/filter persisted Practice workspaces, edit a Practice display name, and suspend or reactivate service access with a recorded reason. Final decommission remains unavailable until its clinical-work, export, retention, and restore preflight exists. The schedule provides a practice-filtered day agenda, appointment details, and explicit non-persistent scheduling states. Clinical workflows remain non-persistent.
- TanStack Query, Zod, and Zustand are installed. Check `web/package.json` and its lockfile for actual dependencies and scripts.
- `api/` contains the Spring Boot/PostgreSQL foundation, framework migrations, a Dockerized local Nginx proxy for `epr.test` and `support.epr.test`, and a local-profile password authentication adapter backed by Spring Session JDBC. The local seeder maintains one configurable `SUPERADMIN` development account; the React app uses the hostname to expose only the clinical or provider-support route family while retaining combined localhost compatibility. The backend includes a development tenant-provisioning and administration slice with a control-plane Practice registry, support-issued one-time administrator passwords or safe existing-account linking, mandatory password replacement at first login, initial membership, idempotent/quarantined provisioning, separate PostgreSQL databases and roles, tenant Flyway bootstrap, first-site persistence, an encrypted local credential adapter, optimistic Practice metadata updates, reversible service suspension, and append-only administration events. No email invitation is sent. Production Secrets Manager/RDS administration, runtime tenant datasource routing, fleet migrations, Cognito/PKCE, MFA, tenant authorization for clinical requests, decommission orchestration, broader audit workflows, and persisted clinical services remain unimplemented. Vitest covers the existing frontend selectors, forms, provider administration contracts, and host/role landing, and JUnit/Testcontainers cover modular boundaries, local authentication, provisioning recovery, administrator selection, first-site persistence, Practice administration lifecycle, and PostgreSQL database isolation; browser checks remain temporary rather than committed.
...
| `docs/architecture/database-per-practice-tenancy.md` | Accepted control-plane, database-per-Practice routing, provisioning, fleet-migration, pooling, restore, and relocation decisions. |
...
| Data | PostgreSQL 17 on Amazon RDS; a control-plane database plus one logical database per Practice tenant from launch; distinct per-tenant runtime/migration credentials; JPA/Hibernate plus explicit SQL where needed; controlled Flyway fleet migrations. |
| Identity/API | Cognito with MFA; separate `app.<product-domain>` and `support.<product-domain>` browser origins and Cognito clients; Spring Security; Spring Session JDBC in the control-plane database; origin-specific secure HttpOnly cookies and CSRF protection; REST/OpenAPI with generated TypeScript contracts. Future native clients use Cognito bearer tokens through a separate security adapter. |
...
- Resolve `practiceId` through an authorized control-plane tenant registry to a server-owned database/secret reference. Never accept a database name, URL, schema or credential selector from the client. Missing, disabled, migrating or mismatched routes fail closed.
- Provision a separate database, runtime role and migration role per Practice. Retain `practice_id` plus tenant/patient-consistent foreign keys within each tenant database for defense-in-depth and audit/export integrity. Runtime roles never own tables or connect to another tenant database. Use `FORCE ROW LEVEL SECURITY` for shared tenant-scoped control-plane or operational tables.
- Bound aggregate tenant datasource connections, lazily open and evict pools, and test route/pool reuse for tenant leakage. Jobs carry an explicit tenant route; application services additionally enforce site, assignment, role, field, and care-relationship restrictions.
- Run Flyway as an observable tenant-fleet operation with canaries, per-tenant schema status, bounded concurrency and a quarantined failure state. Do not assume one migration transaction can span tenant databases.
