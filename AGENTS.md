...
- `api/` contains the Spring Boot/PostgreSQL foundation, framework migrations, a Dockerized local Nginx proxy for `epr.test` and `support.epr.test`, and a local-profile password authentication adapter backed by Spring Session JDBC. The local seeder maintains one configurable `SUPERADMIN` development account; the React app uses the hostname to expose only the clinical or provider-support route family while retaining combined localhost compatibility. The backend now includes a development tenant-provisioning slice with a control-plane Practice registry, initial membership, idempotent/quarantined provisioning, separate PostgreSQL databases and roles, tenant Flyway bootstrap, and an encrypted local credential adapter. Production Secrets Manager/RDS administration, runtime tenant datasource routing, fleet migrations, Cognito/PKCE, MFA, tenant authorization for clinical requests, broader audit workflows, and persisted clinical services remain unimplemented. Vitest covers the existing frontend selectors and form behavior plus host/role landing, and JUnit/Testcontainers cover modular boundaries, local authentication, provisioning recovery, and PostgreSQL database isolation; browser checks remain temporary rather than committed.
