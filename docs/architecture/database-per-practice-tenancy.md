...
The local Spring implementation now has a control-plane Practice registry, support-issued one-time initial-administrator passwords or safe existing-account linking, required private-password replacement at first login, membership with separate multi-role assignments, and append-only provisioning history; idempotent leased provisioning; deterministic server-owned database and role names; distinct non-owning runtime and migration roles; separate tenant Flyway migrations; tenant-identity and first-site verification; quarantined failures; and an encrypted local-only credential adapter. Plaintext temporary passwords are returned once and never persisted, and no email invitation is sent. An initial administrator may be a clinician or non-clinical Practice user, while provider-workforce authority cannot be converted into tenant membership. Superadmin REST endpoints create and inspect Practice tenants without exposing database routes or secrets. PostgreSQL Testcontainers cover retry behavior, unsafe-role rejection, administrator identity conflicts, temporary credential state, initial-site persistence, runtime verification, and denial of cross-tenant database connections.
