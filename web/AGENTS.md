...
The application implements a responsive, synthetic-data Dashboard in `src/features/dashboard/`, doctor schedule workspace in `src/features/schedule/`, My Patients directory plus add/edit forms in `src/features/patients/`, and patient charts plus historical encounter screens in `src/features/chart/`, composed by `src/App.tsx`. React Router uses hash routes for Dashboard, Schedule (`/schedule`), My Patients, patient forms, practice-scoped charts (`/patients/:practiceId/:patientId/chart`), and encounters (`/patients/:practiceId/:patientId/encounters/:encounterId`), with a data-router blocker for unsaved form and encounter-note navigation. Schedule supports a practice-filtered day agenda, expanding fixture rows, appointment selection/details, date empty states, and explicit waitlist and availability preview states; creating and rescheduling remain unavailable without a connected service. Chart review does not start care. Maria’s synthetic historical draft and signed metadata are scoped to Hospital A; other patients have explicit unavailable clinical-data states. Draft edits stay in component memory and are discarded on leaving or reload; saving, signing, starting care, issuing and reconciliation remain unavailable. Signed document bytes are not included in the fixture. Shared application layout and controls live in `src/components/`; global font/color tokens are in `src/index.css`. Tailwind CSS, Lucide, self-hosted Roboto, and shadcn-style Button/Radix Dialog primitives are installed. Vitest covers dashboard/directory and schedule display selectors, form validation, in-memory fixture save behavior, and chart/encounter fixture scoping and note defaults. React, TypeScript, TanStack Query, Zod, and Zustand are also installed. React Router, React Hook Form, and the Zod resolver are installed. There is no backend connection, authentication, or durable clinical workflow. A typed in-memory fixture repository supports explicitly labeled demo add/edit commands, versions, duplicate-review guards, idempotent retries, and transient history; all reset on reload. Playwright browser checks currently run through temporary scripts outside the repository, not a committed e2e suite.
