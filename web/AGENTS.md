...
The application implements a responsive, synthetic-data Dashboard in `src/features/dashboard/`, doctor schedule workspace in `src/features/schedule/`, My Patients directory plus add/edit forms in `src/features/patients/`, patient charts plus historical encounter screens in `src/features/chart/`, and a provider sign-in/protected provider shell in `src/features/auth/` and `src/features/provider/`, composed by `src/App.tsx`. React Router uses hostname-aware hash routes: local Nginx exposes the clinical route family at `epr.test`, the provider-support route family at `support.epr.test`, and combined compatibility on localhost/direct-IP development. Authentication calls the local Spring API through a same-origin `/api` proxy, uses CSRF-protected JSON login/logout, and restores the server-side JDBC session on reload; host-only cookies keep the two local host sessions separate. The production target retains distinct Cognito clients, cookie names, and CSRF boundaries. Only the local `SUPERADMIN` development account is currently seeded. Provider support can create a Practice with a one-time administrator password or safely linked existing account and first PSGC-coded Philippine site, then list/search/filter persisted Practice workspaces, open a detail view, issue a replacement temporary password where eligible, edit the display name, and suspend or reactivate service access with a required reason. A temporary-password account must set a private password before other API access. Provisioning health and service access are separate states; final decommission is visibly unavailable pending its safety preflight. No email invitation is sent, and the UI must not silently turn provider authority or administrative membership into chart access. Cognito/PKCE, MFA, tenant grants, runtime clinical tenant routing, Practice-administrator settings, decommission orchestration, and broader provider operations remain unimplemented. Schedule, chart, and patient behavior remain explicitly synthetic and non-persistent. Vitest covers dashboard/directory and schedule selectors, form validation, in-memory fixture behavior, chart/encounter scoping, provider administration contracts, login validation, and host/role landing. Playwright browser checks still run through temporary scripts outside the repository, not a committed e2e suite.
...
Run `ci` when dependencies need installation, and `build` before preview. `lint` runs Oxlint; `build` runs TypeScript project checks and Vite. Use the actual URL printed by the dev server. `test` runs Vitest dashboard/directory and schedule selectors, form validation, in-memory fixture repository tests, and chart/encounter fixture tests. There is no `test:e2e` or separate `typecheck` script; inspect and update this section when introducing them. Never claim a missing command ran.
