...
- Provider support (`#/provider/practices`): a persisted Practice directory with name/code/ID search, provisioning/service filters, pagination, administrator and schema metadata, and responsive table-to-card reflow. Each detail workspace supports optimistic display-name edits and reasoned, reversible suspension/reactivation; provisioning health remains distinct from service access. Final decommission is deliberately disabled until export, retention, restore, and unresolved-clinical-work checks are implemented.
...
- `src/features/provider/`: authenticated provider shell, persisted Practice creation and administration clients, directory/detail views, and reasoned lifecycle controls.
...
The feature `demo-data.ts` files are explicit synthetic seeds. PatientPreviewProvider and PatientPreviewRepository keep demo additions, edits, revisions, retry outcomes, and before/after history in application memory until reload. Global search uses the same expanded directory identities; birth dates match existing dashboard fixture ages at the snapshot date. The provider login uses the local API's CSRF-protected, JDBC-backed browser session and role guard. The tenant-creation form calls the idempotent local provisioning API, creates an enabled initial-administrator account with a one-time support-handoff password or links a matching eligible existing Practice account, and persists the first clinic location in the new tenant database using the Philippine PSGC hierarchy. New account passwords are shown once, stored only as BCrypt hashes, and must be replaced at first login before other API access. The Practice directory and detail workspace read and mutate control-plane records through the API with TanStack Query, runtime response validation, optimistic versions, and stable idempotency keys for retries. Suspension preserves the tenant database and memberships while making the Practice non-routable; reactivation is explicit and audited. No email invitation is sent; provider staff and Practice administrators receive no chart access from this workflow. Clinicians and their credentials are added separately from tenant creation. Runtime tenant authorization, the Practice-administrator settings workspace, durable clinical persistence, final decommission, live connection status, offline patient storage, and production Cognito boundaries remain unimplemented. Opening a patient does not create an encounter. Results cannot be marked reviewed and notes cannot be signed. No result content or allergy absence is fabricated. Display selector tests do not establish server authorization or readiness for real patients.
