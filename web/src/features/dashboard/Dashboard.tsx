...
import { chartPath, encounterPath } from '../chart/model'
...
export function Dashboard({ view = 'dashboard', onViewChange, content }: { view?: 'dashboard' | 'patients' | 'schedule'; onViewChange: (view: 'dashboard' | 'patients' | 'schedule') => void; content?: ReactNode }) {
  ...
  function navigate(destination: Destination) {
    if (destination === 'dashboard' || destination === 'patients' || destination === 'schedule') { setTab('today'); setPanel(null); onViewChange(destination); return }
